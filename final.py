...
def admin_logs():
    ...
    logs = cursor.execute("SELECT * FROM search_logs").fetchall()
...
def prediction():
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            "INSERT INTO search_logs(username,search_data) VALUES(?,?)",
            (username, "Heart Prediction"))

        conn.commit()

        explanation, suggestion = explain_prediction(data)
        graph = generate_graph(data[0])
        report = generate_report(result, explanation, suggestion)
...
def classify_image():
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            prob = image_model.predict_proba(img)[0][1]
            ...
            cursor.execute(
                "INSERT INTO search_logs(username,search_data) VALUES(?,?)",
                (username, "Image Classification"))

            conn.commit()

            graph = generate_graph(img[0][:6])
            report = generate_report(
    result,
    "Image Classification Result",
    "Maintain healthy lifestyle and consult doctor if symptoms appear"
)
